...
// TEST: Trigger recurring transactions every 30 seconds for testing
export const triggerRecurringTransactionsTest = inngest.createFunction(
  {
    id: "trigger-recurring-transactions-test",
    name: "Trigger Recurring Transactions (Test - 30s)",
  },
  { cron: "*/30 * * * * *" }, // Every 30 seconds (test mode)
  async ({ step }) => {
    const now = new Date();
    console.log(
      `[TEST-trigger] Running at ${now.toISOString()}`
    );

    const recurringTransactions = await step.run(
      "fetch-recurring-transactions-test",
      async () => {
        const results = await db.transaction.findMany({
          where: {
            isRecurring: true,
        ...
        console.log(
          `[TEST-trigger] Found ${results.length} recurring transactions`
        );
        ...
        const dueTx = results.filter((tx) => {
          ...
          if (isDue) {
            console.log(
              `[TEST-trigger] Transaction ${tx.id} is due. nextRecurringDate: ${tx.nextRecurringDate}, lastProcessed: ${tx.lastProcessed}`
            );
          }
          return isDue;
        });

        console.log(`[TEST-trigger] ${dueTx.length} transactions are due`);
        return dueTx;
      }
    );
    ...
    if (recurringTransactions.length > 0) {
      console.log(
        `[TEST-trigger] Sending ${recurringTransactions.length} events to process`
      );
      const events = recurringTransactions.map((transaction) => ({
        name: "transaction.recurring.process",
        data: {
    ...
    console.log(`[TEST-trigger] No recurring transactions to process`);
    return { triggered: 0, success: true };
  }
);
